...
async function buildCheckMacValue(params, hashKey, hashIV) {
  ...
  // Match PHP ksort(): case-sensitive ascending key order (differs from localeCompare).
...
/** ECPay CustomField1–4 max length 50 each */
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/ecpay/create-order' && request.method === 'POST') {
      try {
        return await handleCreateOrder(request, env);
      } catch (e) {
        return json({ error: e?.message || 'Create order failed' }, 500);
      }
    }
